...
public sealed unsafe class CleanupCharacterConfiguration : InventoryManagementBase
{
    ...
    {
        ImGuiEx.TextWrapped(Lang.T($"Here you can assign preconfigured inventory cleanup lists to your registered characters."));
        ImGuiEx.SetNextItemFullWidth();
        ImGuiEx.FilteringInputTextWithHint("##search", Lang.T("Search..."), out var filter);
        if(ImGuiEx.BeginDefaultTable([Lang.T("~Character"), Lang.T("Plan")]))
        {
            foreach(var characterData in C.OfflineData)
            {
                ...
                {
                    if(ImGui.Selectable(Lang.T("Default Plan"), plan == null)) characterData.InventoryCleanupPlan = Guid.Empty;
                    ImGui.Separator();
