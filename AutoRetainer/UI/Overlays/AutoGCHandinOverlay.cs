...
internal unsafe class AutoGCHandinOverlay : Window
{
    ...
    public override void Draw()
    {
        if(Allowed)
        {
            ImGui.Checkbox("啟用自動稀有品繳納", ref AutoGCHandin.Operation);
        }
        ...
        {
            ImGui.SameLine();
            ImGuiEx.Text(GradientColor.Get(ImGuiColors.DalamudRed, ImGuiColors.DalamudYellow), $"可使用軍票優先支給券");
        }
        if(!Player.IsInHomeWorld)
        {
            ImGui.SameLine();
            ImGuiEx.Text(GradientColor.Get(ImGuiColors.DalamudRed, ImGuiColors.DalamudYellow), $"目前在外部伺服器，不會獲得部隊點數。");
        }
        if(AutoGCHandin.Operation && Remaining != 0)
        {
            ImGui.SameLine();
            ImGuiEx.Text(ImGuiColors.DalamudViolet, $"剩餘 {Remaining} 件物品");
        }
